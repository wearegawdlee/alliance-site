...
async function recordPayment(invoiceId, data) {
  ...
  try {
    ...
    const paidStatus = await getInvoiceStatusId(
      client,
      Number(paid) >= Number(invoice.total_amount) ? "paid" : "partially_paid",
    );
    await client.query(
      `UPDATE invoices SET invoice_status_id=$1,updated_at=current_timestamp WHERE id=$2`,
      [paidStatus, invoiceId],
    );
    if (Number(paid) >= Number(invoice.total_amount) && invoice.work_order_id) {
      const closed = (
        await client.query(
          `SELECT id FROM work_order_statuses WHERE code='closed'`,
        )
      ).rows[0]?.id;
      if (closed) {
        await client.query(
          `UPDATE work_orders SET work_order_status_id=$1,updated_at=current_timestamp WHERE id=$2`,
          [closed, invoice.work_order_id],
        );
        await client.query(
          `INSERT INTO work_order_status_history(work_order_id,to_status_id,reason) VALUES($1,$2,'Invoice paid; ticket closed')`,
          [invoice.work_order_id, closed],
        );
      }
    }
