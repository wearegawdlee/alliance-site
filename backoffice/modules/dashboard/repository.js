...
async function getDashboard() {
  const [
    ...
    pool.query(`SELECT wo.id,wo.title,wo.scheduled_start_at,c.display_name customer,wos.code status_code,wos.name status,sl.name service_line,
      ...
      WHERE wos.code NOT IN ('closed','cancelled')
        AND (wos.code IN ('requested','contacted','estimate_pending','completed','invoiced') OR woa.id IS NULL)
      GROUP BY wo.id,wo.title,wo.scheduled_start_at,c.display_name,wos.code,wos.name,sl.name
      ORDER BY CASE WHEN wos.code='requested' THEN 1 WHEN COUNT(woa.id)=0 THEN 2 WHEN wos.code='estimate_pending' THEN 3 WHEN wos.code='completed' THEN 4 ELSE 5 END, COALESCE(wo.scheduled_start_at,wo.created_at) ASC LIMIT 12`),
    pool.query(`SELECT wo.id,wo.title,wo.scheduled_start_at,c.display_name customer,wos.name status,string_agg(DISTINCT u.display_name, ', ' ORDER BY u.display_name) assigned_to
      ...
      LEFT JOIN work_order_assignments woa ON woa.work_order_id=wo.id
      LEFT JOIN users u ON u.id=woa.user_id
      WHERE wo.scheduled_start_at::date = current_date AND wos.code NOT IN ('closed','cancelled')
