...
async function findServiceLineId(client, rawServiceType) {
  ...
  if (/pool/.test(normalized))
    return findIdByCode(client, "service_lines", "pools");
  if (/screen/.test(normalized))
    return findIdByCode(client, "service_lines", "motorized_screens");
...
async function createWebsiteLead(data) {
  ...
  try {
    ...
    const prospectStatusId = await findIdByCode(
      client,
      "customer_statuses",
      "prospect",
    );
    ...
    const displayName =
      data.name || data.email || data.phone || "Website Prospect";
