...
function normalizeLeadPayload(body, req) {
  return {
    ...
    preferred_contact_method: clean(
      body.preferred_contact_method || body.preferredContactMethod,
    ),
    address_line_1: clean(
      body.address || body.address_line_1 || body.addressLine1,
    ),
    city: clean(body.city),
    state: clean(body.state) || "GA",
    postal_code: clean(
      body.zip || body.zip_code || body.postal_code || body.postalCode,
    ),
...
async function createWebsiteLead(body, req) {
  ...
  notifications.notifyWebsiteLead({
