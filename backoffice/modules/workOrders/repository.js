...
async function listWorkOrders(filters = {}) {
  ...
  if (filters.assigned_user_id) {
    if (filters.assigned_user_id === 'unassigned') {
      where.push(`NOT EXISTS (SELECT 1 FROM work_order_assignments ax WHERE ax.work_order_id = wo.id)`);
    } else {
      params.push(Number(filters.assigned_user_id));
      where.push(`EXISTS (SELECT 1 FROM work_order_assignments ax WHERE ax.work_order_id = wo.id AND ax.user_id = $${params.length})`);
    }
  }
...
async function getWorkOrderDetail(id) {
  const wr = await pool.query(`SELECT wo.*,c.display_name customer,wos.code status_code,wos.name status,sl.name service_line,wot.name work_order_type,cl.label location_label,cl.address_line_1,cl.city,cl.state,cl.postal_code,a.name asset FROM work_orders wo JOIN customers c ON c.id=wo.customer_id JOIN work_order_statuses wos ON wos.id=wo.work_order_status_id JOIN service_lines sl ON sl.id=wo.service_line_id JOIN customer_locations cl ON cl.id=wo.customer_location_id LEFT JOIN work_order_types wot ON wot.id=wo.work_order_type_id LEFT JOIN assets a ON a.id=wo.asset_id WHERE wo.id=$1`, [id]);
  ...
  const [statuses, users, assignments, notes, history, invoices, lineItems, catalogItems] = await Promise.all([
    ...
    pool.query(`SELECT i.*,s.name status FROM invoices i JOIN invoice_statuses s ON s.id=i.invoice_status_id WHERE i.work_order_id=$1 ORDER BY i.created_at DESC`, [id]),
  ...
  return {
    ...
    lineItemTotal: lineItems.rows.reduce((sum, item) => sum + Number(item.line_total || 0), 0)
  };
}
...
async function updateWorkOrder(id, data, userId) {
  ...
  try {
    ...
    await client.query(`UPDATE work_orders SET work_order_status_id=$1,scheduled_start_at=$2,quoted_price=$3,final_price=$4,updated_at=current_timestamp WHERE id=$5`, [data.work_order_status_id, data.scheduled_start_at || null, data.quoted_price || null, data.final_price || null, id]);
    if (Number(cur.rows[0].work_order_status_id) !== Number(data.work_order_status_id)) {
      await client.query(`INSERT INTO work_order_status_history(work_order_id,from_status_id,to_status_id,changed_by_user_id,reason) VALUES($1,$2,$3,$4,$5)`, [id, cur.rows[0].work_order_status_id, data.work_order_status_id, userId || null, data.reason || 'Manual work order update']);
    }
...
async function transition(id, action, userId, reason) {
  ...
  try {
    ...
    const cur = await client.query(`SELECT work_order_status_id FROM work_orders WHERE id=$1`, [id]);
    if (!cur.rows[0]) throw new Error('Work order not found');
    const map = { contact: 'contacted', schedule: 'scheduled', assign: 'assigned', evaluate: 'in_evaluation', estimate: 'estimate_pending', approve: 'approved', start: 'in_progress', complete: 'completed', invoice: 'invoiced', close: 'closed', cancel: 'cancelled' };
...
async function addNote(id, userId, note) {
  await pool.query(`INSERT INTO work_order_notes(work_order_id,author_user_id,note_body) VALUES($1,$2,$3)`, [id, userId || null, note]);
}
...
async function deleteLineItem(workOrderId, lineItemId) {
  await pool.query(`DELETE FROM work_order_line_items WHERE id=$1 AND work_order_id=$2`, [lineItemId, workOrderId]);
}
...
async function generateInvoice(id, userId, data) {
  ...
  try {
    ...
    const lineTotal = lineItems.reduce((sum, item) => sum + Number(item.line_total || 0), 0);
    const manualTotal = Number(data.total_amount || wo.final_price || wo.quoted_price || 0);
    const total = lineItems.length ? lineTotal : manualTotal;
    const inv = await client.query(`INSERT INTO invoices(customer_id,customer_location_id,work_order_id,invoice_status_id,invoice_number,issue_date,due_date,subtotal,total_amount,notes) VALUES($1,$2,$3,$4,$5,current_date,current_date + interval '14 days',$6,$6,$7) RETURNING id`, [wo.customer_id, wo.customer_location_id, id, draft, `INV-${Date.now()}`, total, data.notes || null]);
    if (lineItems.length) {
      for (const item of lineItems) {
        await client.query(`INSERT INTO invoice_line_items(invoice_id,catalog_item_id,description,quantity,unit_price,line_total) VALUES($1,$2,$3,$4,$5,$6)`, [inv.rows[0].id, item.catalog_item_id, item.description, item.quantity, item.unit_price, item.line_total]);
      }
    } else {
      await client.query(`INSERT INTO invoice_line_items(invoice_id,description,quantity,unit_price,line_total) VALUES($1,$2,1,$3,$3)`, [inv.rows[0].id, data.description || wo.title, total]);
    }
    if (data.mark_completed) {
      const invoiced = await getStatusId(client, 'invoiced');
      await client.query(`UPDATE work_orders SET work_order_status_id=$1,final_price=COALESCE(final_price,$2),updated_at=current_timestamp WHERE id=$3`, [invoiced, total, id]);
      await client.query(`INSERT INTO work_order_status_history(work_order_id,to_status_id,changed_by_user_id,reason) VALUES($1,$2,$3,'Invoice generated')`, [id, invoiced, userId || null]);
    }
    await client.query('COMMIT'); return inv.rows[0].id;
  } catch (e) { await client.query('ROLLBACK'); throw e; } finally { client.release(); }
}
