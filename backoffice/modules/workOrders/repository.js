...
async function getWorkOrderDetail(id){
  ...
  const [statuses,users,assignments,notes,history,invoices]=await Promise.all([
  ...
  return {...wr.rows[0],statuses:statuses.rows,users:users.rows,assignments:assignments.rows,notes:notes.rows,history:history.rows,invoices:invoices.rows};
}
...
async function generateInvoice(id,userId,data){
  ...
  try{
    ...
    const total=Number(data.total_amount || wo.final_price || wo.quoted_price || 0);
    ...
    if(data.mark_completed){ const invoiced=await getStatusId(client,'invoiced'); await client.query(`UPDATE work_orders SET work_order_status_id=$1,final_price=COALESCE(final_price,$2),updated_at=current_timestamp WHERE id=$3`,[invoiced,total,id]); await client.query(`INSERT INTO work_order_status_history(work_order_id,to_status_id,changed_by_user_id,reason) VALUES($1,$2,$3,'Invoice generated')`,[id,invoiced,userId||null]); }
