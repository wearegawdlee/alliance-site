...
async function listWorkOrders(query) {
  return repo.listWorkOrders(query || {});
}
async function getWorkOrderFilters() {
  return repo.getWorkOrderFilters();
}
async function getWorkOrderDetail(id) {
  return repo.getWorkOrderDetail(id);
}
async function updateWorkOrder(id, body, user) {
  await repo.updateWorkOrder(
    id,
    {
      work_order_status_id: Number(body.work_order_status_id),
...
async function transition(id, body, user) {
  return repo.transition(id, body.action, user?.id, clean(body.reason));
}
async function addNote(id, userId, note) {
  note = clean(note);
  if (note) await repo.addNote(id, userId, note);
}
async function addLineItem(id, body) {
  const description = clean(body.description);
  const quantity = Number(body.quantity || 1);
  const unit_price =
    body.unit_price === "" || body.unit_price === undefined
      ? null
      : Number(body.unit_price);
  const catalog_item_id = body.catalog_item_id
    ? Number(body.catalog_item_id)
    : null;
  if (!description && !catalog_item_id)
    throw new Error("Line item description is required");
  return repo.addLineItem(id, {
    catalog_item_id,
    description,
    quantity,
    unit_price,
...
async function deleteLineItem(id, lineItemId) {
  return repo.deleteLineItem(id, lineItemId);
}
async function generateInvoice(id, body, user) {
  const invoiceId = await repo.generateInvoice(id, user?.id, {
