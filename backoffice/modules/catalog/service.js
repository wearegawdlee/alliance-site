...
async function createCatalogItem(body){const data=payload(body); if(!data.name) throw new Error('Catalog item name is required'); return repo.createCatalogItem(data);}
async function updateCatalogItem(id,body){const data=payload(body); if(!data.name) throw new Error('Catalog item name is required'); return repo.updateCatalogItem(id,data);}
