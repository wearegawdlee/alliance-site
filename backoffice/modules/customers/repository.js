...
async function updateCustomer(id,data){
  ...
  try{
    ...
    if(loc.rows[0]) await client.query(`UPDATE customer_locations SET property_type_id=$1,label=$2,address_line_1=$3,city=$4,state=$5,postal_code=$6,gate_code=$7,access_notes=$8,updated_at=current_timestamp WHERE id=$9`,[data.property_type_id||null,data.location_label||'Service Location',data.address_line_1||null,data.city||null,data.state||null,data.postal_code||null,data.gate_code||null,data.access_notes||null,loc.rows[0].id]);
...
async function transitionCustomer(id, action, userId, reason){
  ...
  try{
    ...
    if(action==='contacted'){ toCode='qualified_lead'; eventType='lead.contacted'; eventBody='Lead picked up / contact made'; }
...
async function createWorkOrderFromCustomer(customerId,data,userId){
  ...
  try{
    ...
    const requestedStatus=await getWorkOrderStatusId(client,data.assigned_user_id ? 'scheduled' : 'requested');
