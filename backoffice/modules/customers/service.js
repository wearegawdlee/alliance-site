...
function normalizeCustomer(body, user) {
  return {
    display_name: clean(body.display_name),
    customer_status_id: Number(body.customer_status_id),
    lead_source_id: body.lead_source_id ? Number(body.lead_source_id) : null,
    ...
    property_type_id: body.property_type_id
      ...
      : null,
    location_label: clean(body.location_label) || "Service Location",
    address_line_1: clean(body.address_line_1),
    gate_code: clean(body.gate_code),
    access_notes: clean(body.access_notes),
    city: clean(body.city),
    state: clean(body.state) || "GA",
    postal_code: clean(body.postal_code),
    service_line_ids: toArray(body.service_line_ids)
      .map(Number)
      .filter(Boolean),
    initial_note: clean(body.initial_note),
    author_user_id: user && user.id,
  };
}
async function listCustomers(filters) {
  return repo.listCustomers(filters || {});
}
async function getCustomerDetail(id) {
  return repo.getCustomerDetail(id);
}
async function getFormOptions() {
  return repo.getFormOptions();
}
async function createCustomer(body, user) {
  return repo.createCustomer(normalizeCustomer(body, user));
}
async function updateCustomer(id, body, user) {
  return repo.updateCustomer(id, normalizeCustomer(body, user));
}
async function deleteCustomer(id, user) {
  return repo.deleteCustomer(id, user?.id);
}
async function transitionCustomer(id, body, user) {
  return repo.transitionCustomer(id, body.action, user?.id, clean(body.reason));
}
async function createWorkOrderFromCustomer(customerId, body, user) {
  ...
  return repo.createWorkOrderFromCustomer(
    customerId,
    {
      title,
      ...
      work_order_type_id: body.work_order_type_id
        ? Number(body.work_order_type_id)
        : null,
      customer_location_id: body.customer_location_id
        ? Number(body.customer_location_id)
        : null,
...
async function addNote(customerId, userId, note) {
  ...
  if (note) await repo.addNote(customerId, userId, note);
}
module.exports = {
  listCustomers,
  getCustomerDetail,
